...
export default function OrderConfirmationPage() {
    ...
    }, [user, item, router, toast]);

    const handleConfirmOrder = () => {
        if (!item) return;
        setIsConfirming(true);
        const orderId = confirmOrder(item.id);
        if (orderId) {
            toast({
    ...
    // For now, we'll use the user's last known submission location as delivery address
    const userLastSubmission = submissions.find(s => s.userId === user.id);
    const deliveryAddress = userLastSubmission?.location.address || "Please set a default address.";
            ...
            <Card>
                ...
                <CardFooter className="flex-col gap-2">
                    <div className="w-full flex justify-between items-center p-4 bg-accent/30 rounded-lg">
                     ...
                     <Button className="w-full" size="lg" onClick={handleConfirmOrder} disabled={isConfirming}>
